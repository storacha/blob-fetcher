...
import { generateIndexClaim, generateLocationClaim, generateLocationClaims, withClaimsServer, withTestIndexer } from './helpers/claims.js'
...
/**
 * @typedef {import('./helpers/bucket.js').BucketServerContext & import('./helpers/claims.js').IndexingServerContext & import('./helpers/claims.js').ClaimsServerContext } Context
 * @param {(assert: import('entail').Assert, ctx: Context) => unknown} testfn
 */
const withContext = testfn => withBucketServer(withClaimsServer(withTestIndexer(testfn)))
...
;[
  ...
  [
    ...
    testFetcher[`${name} with ${locatorName}`] = {
      'should fetch a file': withContext(async (assert, ctx) => {
        ...
        const locator = LocatorFactory(ctx)
